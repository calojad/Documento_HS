...
//        $this->tempDocumentHeaders = str_replace('<w:t>' . $strKey . '</w:t>', $toAddImg, $this->tempDocumentHeaders);
        ...
        foreach($this->tempDocumentHeaders as $index=>$header){
            $count = 0;
            $this->tempDocumentHeaders[$index] = str_replace($strKey, $toAddImg, $this->tempDocumentHeaders[$index], $count);
            if ($count > 0){
                //check if not set
...
//                    $this->_header_rels[$index]=$this->blank_rels_page;
                ...
                //add
                $this->_header_rels[$index] = str_replace('</Relationships>', $toAddImg, $this->_header_rels[$index]) . '</Relationships>';
            }
        }
