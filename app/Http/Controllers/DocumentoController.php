...
//      INGRESAR VALORES DE LAS VARIABLES
        ...
        $templateWord->setValue('poblacion',$empresa->hombres+$empresa->mujeres+$empresa->menores+$empresa->vulnerables);
...
//      AMBITOS
        $templateWord->cloneRow('ambitos',count($ambitos));
        $i = 0;
        foreach($ambitos as $amb){
            $i++;
            $templateWord->setValue('ambitos#'.$i,$amb->ambitos->descripcion);
        }
//      POLITICAS
